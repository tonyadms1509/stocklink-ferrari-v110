...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const executeAIRequest = async (requestFn: () => Promise<any>, retries = 3, delay = 1000): Promise<any> => {
        if (isGridSaturated) {
        ...
        try {
            ...
            if (errorBody.includes("429") || errorBody.toLowerCase().includes("quota")) {
                setIsGridSaturated(true);
                showToast("National Grid Saturated (429). Resetting nodes...", "warning");
                setTimeout(() => setIsGridSaturated(false), 30000); 
    ...
    const value: DataContextState = {
        ...
        systemStatus, dailyBriefing, aiErrors, isAILoading, cart, favorites, aiDraftedResponses, customerInsights,
        ...
        getDeliveryUpdate: async (oid, q) => { return "Simulated update."; },
        ...
        updateRentalEquipment: async (id, data) => { },
        ...
        releaseMilestoneEscrow: async (id) => { setProjectMilestones(prev => prev.map(m => m.id === id ? { ...m, status: 'Released' } : m)); showToast("Settled", "success"); },
        ...
        addMaterialToProject: async (pid, p, q) => { showToast("Allocated", "success"); },
        submitMilestoneProof: async (id, u, s) => { showToast("Audit complete", "success"); },
        verifyUserNode: (id, s) => api.verifyUserNode(id, s),
        ...
        requestBusinessVerification: async (d) => {},
        ...
        resolveIntervention: async (id) => {
            ...
            showToast("Intervention Resolved", "success");
        },
        ...
        reconcileMaterials: async (projectId, items) => {
            setProjectMaterials(prev => prev.map(mat => {
                if (mat.projectId !== projectId) return mat;
                const match = items.find(i => mat.productName.toLowerCase().includes(i.name.toLowerCase()));
                if (match) {
                    return { ...mat, status: 'Delivered' };
                }
                return mat;
            }));
            showToast("Neural Reconciliation: Handover Complete", "success");
        },
        generateCostEstimate: async (desc) => {
            setIsAILoading(prev => ({ ...prev, estimator: true }));
            try {
                const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
                const response = await ai.models.generateContent({
                    model: 'gemini-3-flash-preview',
                    contents: `Estimate: ${desc}`
                });
                setCostEstimateReport({ description: desc, analysis: response.text });
            } finally { setIsAILoading(prev => ({ ...prev, estimator: false })); }
        },
        generateProjectPlan: async (description) => {
            setIsAILoading(prev => ({ ...prev, planner: true }));
            try {
                const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
                const response = await ai.models.generateContent({
                    model: 'gemini-3-flash-preview',
                ...
                setProjectPlan({ phases: [], safetyChecklist: [], disclaimer: response.text || '' });
            } finally { setIsAILoading(prev => ({ ...prev, planner: false })); }
        },
        generateMarketPulse: async (query) => {
            setIsMarketPulseLoading(true);
            try {
                const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
                const response = await ai.models.generateContent({
                    model: 'gemini-3-flash-preview',
                    contents: `Pulse: ${query}`
                });
                showToast("Market Pulse Captured", "success");
            } finally { setIsMarketPulseLoading(false); }
        },
        performDNAAudit: async (imageFile) => {
            setIsAILoading(prev => ({ ...prev, dna: true }));
            try {
                const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
                const { mimeType, data } = await (async (file: File) => {
                    return new Promise((resolve, reject) => {
                        ...
                        reader.onload = () => {
                            ...
                            else reject(new Error("File error"));
                        };
                    }) as any;
                })(imageFile);
                ...
                const response = await ai.models.generateContent({
                    model: 'gemini-3-flash-preview',
                    contents: { parts: [{ inlineData: { mimeType, data } }, { text: "Audit Structural DNA: rebar count, density, SANS 10400 Part J. Respond in JSON." }] },
                    config: { responseMimeType: 'application/json' }
                });
                return JSON.parse(response.text || '{}');
            } finally { setIsAILoading(prev => ({ ...prev, dna: false })); }
        },
        ...
        deleteTimesheetEntry: async (id) => { await api.deleteTimesheetEntry(id); },
        redeemReward: (offer) => {},
