        ...
        <div className={`relative w-full h-64 rounded-[3rem] p-10 text-white shadow-2xl overflow-hidden transform transition-all hover:scale-[1.02] duration-700 ${isPremium ? 'bg-slate-900 border border-white/10' : isLogistics ? 'bg-gradient-to-br from-cyan-600 to-indigo-800 border border-cyan-400/30' : 'bg-gradient-to-br from-blue-600 to-indigo-900 border border-blue-400/30'}`}>
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
                <div className="flex justify-between items-start">
                    ...
                    <div className="italic font-black text-2xl opacity-20 tracking-tighter text-white">FERRARI</div>
                </div>
                ...
                <div className="flex items-center gap-8">
                    ...
                    <div>
                         <span className="block text-[9px] font-black uppercase tracking-[0.4em] text-white/40 mb-1">Terminal Auth Signature</span>
                         <span className="font-mono text-lg tracking-[0.2em] font-black text-white/90">NODE_ID_{user.id.slice(-8).toUpperCase()}</span>
                    </div>
                </div>
                ...
                <div className="flex justify-between items-end border-t border-white/10 pt-6">
                    ...
                    <div className="text-right">
                        <p className="text-[9px] font-black uppercase text-white/40 mb-1">State</p>
                        <p className="font-black text-xs uppercase text-emerald-400 tracking-widest">ENCRYPTED</p>
                    </div>
                </div>
            </div>
        </div>
...
const PayPalButton: React.FC<{ price: number; onSuccess: () => void }> = ({ price, onSuccess }) => {
    const containerRef = useRef<HTMLDivElement>(null);
    const [loaded, setLoaded] = useState(false);

    useEffect(() => {
        const scriptId = 'paypal-js-sdk-billing';
    ...
    useEffect(() => {
        if (loaded && (window as any).paypal && containerRef.current) {
...
const BillingPage: React.FC = () => {
    ...
    const handleActivation = () => {
        if (gateway === 'PayPal') return; // Handled by PayPalButton component
        ...
        const handler = win.PaystackPop.setup({
            key: 'pk_test_537c72530013553874157216c053021614313414',
            email: user.email,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                ...
                <div className="lg:col-span-4 space-y-10">
                    ...
                    {currentCompany.subscriptionStatus === 'active' ? (
                        <div className="bg-slate-900 border border-emerald-500/20 rounded-[3rem] p-10 text-white relative overflow-hidden group shadow-2xl text-left">
                         ...
                         <div className="bg-slate-900 border border-amber-500/30 rounded-[3rem] p-10 relative overflow-hidden group text-left">
                ...
                <div className="lg:col-span-8">
                    <div className="bg-white rounded-[4rem] shadow-2xl border border-slate-200 p-12 relative overflow-hidden text-left">
                            ...
                            <>
                                ...
                                {gateway === 'Paystack' ? (
                                    ...
                                    >
                                        ...
                                        Execute Paystack Handshake
                                    </button>
                                ) : (
                                    <div className="animate-fade-in-up">
                                        <PayPalButton price={currentPrice} onSuccess={() => { subscribeUser(); showToast("PayPal Global Node Connected", "success"); }} />
                                    </div>
                                )}
                            </>
                        ) : (
                            <div className="bg-emerald-50 border-2 border-emerald-500/20 p-8 rounded-[3rem] text-center">
                                <p className="text-emerald-700 font-bold italic">Node Tier synchronized. Your next settlement is automated via the grid.</p>
                            </div>
                        )}

                        <p className="mt-8 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Current value: {formatCurrency(currentPrice)}</p>
                    </div>
                </div>
            </div>
             ...
             <div className="mt-20 text-left">
                ...
                <div className="bg-slate-900 border border-white/5 rounded-[4rem] p-24 text-center flex flex-col items-center shadow-inner">
                    ...
                    <p className="text-slate-600 font-black uppercase tracking-[0.6em] text-[10px]">No historical fiscal events identified on grid.</p>
                </div>
            </div>
