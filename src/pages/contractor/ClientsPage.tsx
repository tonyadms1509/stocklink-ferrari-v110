...
import { Client, InvoiceStatus, ContractorTab } from '../../types';
...
const ClientDetailView: React.FC<{ client: Client; onBack: () => void; onEdit: () => void; onDelete: () => void }> = ({ client, onBack, onEdit, onDelete }) => {
    ...
    const clientProjects = useMemo(() => projects.filter(p => p.clientName.toLowerCase() === client.name.toLowerCase()), [projects, client]);
    const clientInvoicesList = useMemo(() => clientInvoices.filter(i => i.clientName.toLowerCase() === client.name.toLowerCase()), [clientInvoices, client]);
