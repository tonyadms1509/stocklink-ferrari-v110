...
import { Lead, LeadType, ContractorLeadType } from '../../types';
...
const SalesLeadsPage: React.FC<SalesLeadsPageProps> = ({ onNavigate }) => {
    // leads comes from useData which returns contractorLeads (Lead[])
    const { leads } = useData();
    ...
    return (
        <div className="text-left">
            <h2 className="text-3xl font-bold">{t('navSalesLeads')}</h2>
            <p className="text-gray-600 mb-6">{t('leadsDescription')}</p>
            ...
            {leads.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {leads.map(lead => {
                        ...
                        return (
                            <div key={lead.id} className={`bg-white rounded-lg shadow-md p-6 flex flex-col h-full justify-between group transition-shadow duration-300 hover:shadow-xl border-t-4 ${styles.card}`}>
                                <div className="text-left">
                                    ...
                                    {/* Fixed: Use summary since ContractorLead doesn't have description */}
                                    <p className="text-sm text-gray-600 mt-2 flex-grow">{lead.summary}</p>
                                </div>
                                <button
                                    onClick={() => handleActionClick(lead)}
                                    className="mt-6 w-full bg-slate-900 text-white font-bold py-2.5 px-3 rounded-lg text-sm flex items-center justify-center gap-2 hover:bg-black transition-colors"
                                >
                                    {lead.action.label}
