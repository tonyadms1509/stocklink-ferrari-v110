...
import { XMarkIcon, SparklesIcon, TruckIcon, ExclamationTriangleIcon, CheckCircleIcon, CubeIcon, ArrowPathIcon, ShieldCheckIcon } from '@heroicons/react/24/solid';
                ...
                <div className="p-8 border-b flex justify-between items-center bg-slate-900 text-white shrink-0">
                    <div>
                        <h2 className="text-2xl font-black italic uppercase tracking-tighter flex items-center gap-4 leading-none">
                            <SparklesIcon className="h-8 w-8 text-blue-500 animate-pulse"/> PAYLOAD <span className="text-blue-500">MASTER</span>
                        </h2>
                        <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.4em] mt-3">Mission Optimization v1.2</p>
                    </div>
                    <button onClick={onClose} className="p-3 bg-white/5 rounded-2xl hover:bg-red-600 transition-all"><XMarkIcon className="h-6 w-6"/></button>
                </div>
                ...
                <div className="p-10 overflow-y-auto custom-scrollbar flex-grow">
                    {!strategy && !isLoading && (
                        <div className="space-y-10 animate-fade-in-up">
                            ...
                            >
                                <TruckIcon className="h-6 w-6 text-blue-400 animate-pulse"/>
                    ...
                    {isLoading && (
                        <div className="flex flex-col items-center justify-center py-24">
                            <div className="relative mb-12">
                                ...
                                <SparklesIcon className="h-10 w-10 text-blue-500 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse"/>
                            </div>
                    ...
                    {strategy && (
                        <div className="space-y-12 animate-fade-in-up text-left">
                            ...
                            <div className="bg-red-50 p-10 rounded-[4rem] border border-red-100 shadow-sm relative overflow-hidden">
                                <div className="absolute top-0 right-0 p-8 opacity-5"><ExclamationTriangleIcon className="h-48 w-48 text-red-600" /></div>
                                <h4 className="font-black text-red-900 uppercase text-[10px] tracking-widest mb-8 flex items-center gap-3">
                                    <ShieldCheckIcon className="h-5 w-5 text-red-600"/> Stability Warnings
                                </h4>
                            ...
                            <div className="flex gap-4 pt-10 border-t border-slate-100">
                                <button onClick={() => setStrategy(null)} className="flex-1 py-5 bg-slate-100 hover:bg-white text-slate-500 font-black uppercase text-[10px] tracking-widest rounded-3xl transition-all">Abadon Schematic</button>
