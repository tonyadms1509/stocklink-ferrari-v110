...
const AIAssistantModal: React.FC<AIAssistantModalProps> = ({ isOpen, onClose, onSearch, initialQuery }) => {
  ...
  useEffect(() => {
    ...
    return () => {
        if (abortControllerRef.current) abortControllerRef.current.abort("Modal closed");
    };
  }, [isOpen, initialQuery]);
  ...
  const handleAskAI = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const uniqueCategories = [...new Set(products.map(p => p.category))].slice(0, 20).join(', ');
      ...
      Your goal is to convert natural language requests into specific, technical search terms.
      
      CONTEXT:
      - Available Brands: ${uniqueBrands}
      - Available Categories: ${uniqueCategories}
      
      Respond with ONLY the single most relevant technical search term. No conversational filler.`;
