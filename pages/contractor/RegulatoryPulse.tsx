...
const MessageBubble: React.FC<{ role: 'user' | 'model', text: string }> = ({ role, text }) => (
    <div className={`flex ${role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
        <div className={`max-w-[85%] p-8 rounded-[3rem] ${role === 'user' ? 'bg-red-600 text-white rounded-tr-none shadow-2xl' : 'bg-slate-900 border border-white/10 text-slate-100 rounded-tl-none shadow-2xl'}`}>
             {role === 'model' && (
                 <div className="flex items-center gap-3 mb-4 pb-4 border-b border-white/5">
                    <div className="w-1.5 h-6 bg-blue-600 rounded-full shadow-[0_0_10px_#2563eb]"></div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-blue-400">Neural Logic Pulse: Universal Standard Protocol</span>
                 </div>
             )}
...
const RegulatoryPulse: React.FC = () => {
    ...
    return (
        <div className="pb-12 max-w-6xl mx-auto h-[calc(100vh-250px)] flex flex-col font-sans selection:bg-red-600/30">
             <div className="flex flex-col md:flex-row justify-between items-end gap-10 border-b border-white/5 pb-12 mb-12 text-left">
                <div>
                    ...
                    <h1 className="text-7xl font-black tracking-tighter italic uppercase text-white leading-none">REGULATORY <span className="text-blue-600">PULSE</span></h1>
                </div>
                
                <div className="flex items-center gap-6 bg-white/5 p-2 rounded-[2rem] border border-white/5 backdrop-blur-xl shadow-2xl">
                    <div className="flex flex-col gap-1 px-4 text-left">
                        <label className="text-[8px] font-black uppercase text-slate-500 tracking-widest">Protocol Region</label>
                        <select 
                            value={jurisdiction}
                    ...
                    <div className="flex flex-col gap-1 px-4 text-left">
                        <label className="text-[8px] font-black uppercase text-slate-500 tracking-widest">Standard Logic</label>
                        <select 
                            value={standard}
            ...
            <div className="flex-grow bg-slate-900/50 rounded-[4rem] border border-white/5 shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden relative backdrop-blur-md">
                ...
                <div className="p-10 bg-black/40 border-t border-white/5 backdrop-blur-2xl z-20">
                     <form onSubmit={handleQuery} className="relative">
                         <input 
                            ...
                            className="w-full p-8 pr-24 bg-white/5 border-2 border-white/5 rounded-[3rem] font-black text-white italic text-xl placeholder-slate-700 focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 outline-none transition-all shadow-inner tracking-tighter"
                         />
                         <button 
                            type="submit"
            ...
            <div className="fixed bottom-10 left-10 pointer-events-none opacity-[0.03] -z-10 rotate-90 select-none text-left">
                <span className="text-[160px] font-black tracking-tighter text-white uppercase italic leading-none">REGULATORY_HUB</span>
            </div>
        </div>
    );
};
