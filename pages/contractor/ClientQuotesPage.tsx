                ...
                <div className="p-8 md:p-12 flex-grow font-serif text-gray-800">
                    ...
                    <div className="flex justify-between items-start border-b-2 border-gray-800 pb-8 mb-8">
                        <div>
                            ...
                            <p className="text-gray-500 font-sans font-bold">{user?.name}</p>
                            <p className="text-gray-500 text-sm font-sans">{user?.email}</p>
                            <p className="text-gray-500 text-sm font-sans">{user?.contact?.phone}</p>
                        </div>
                        <div className="text-right">
                            <div className="bg-gray-100 p-4 rounded-lg border border-gray-200">
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Quote For</label>
                                <input 
                                    type="text" 
                                ...
                                <input 
                                    type="text" 
                                    value={editedQuote.clientEmail || ''} 
                                    onChange={e => handleFieldChange('clientEmail', e.target.value)} 
                                    className="w-full bg-transparent border-b border-gray-300 focus:border-primary focus:outline-none text-sm text-right mt-2 placeholder-gray-400"
                    ...
                    <div className="mb-8">
                        ...
                        <div className="space-y-2 font-sans">
                            {editedQuote.items.map((item, index) => (
                                <div key={item.id} className="grid grid-cols-12 gap-4 items-center group hover:bg-gray-50 p-2 rounded transition-colors relative text-left">
...
const ClientQuotesPage: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
    ...
    const handleCreateNew = () => {
        ...
        setEditingQuote({
            ...
            items: [],
            vatRate: 0.15,
            /* Fix: Added missing total property */
            total: 0,
    ...
    const handleImportQuote = (items: { productName: string; quantity: number; pricePerUnit: number }[]) => {
         ...
         /* Fix: Added required total property to initial import state */
