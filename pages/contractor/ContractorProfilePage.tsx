...
import WebsitePreviewModal from '../../components/WebsitePreviewModal.tsx';
            ...
            <div className="relative z-10">
                ...
                <div className="space-y-4">
                    ...
                    <div className={`h-1 w-full rounded-full overflow-hidden border ${isRuggedMode ? 'bg-white border-black border-2' : 'bg-white/5 border-white/5'}`}>
                        <div className={`h-full ${isRuggedMode ? 'bg-black' : 'bg-emerald-50'}`} style={{ width: '100%' }}></div>
                    </div>
                </div>
            </div>
        </div>
...
const ContractorProfilePage: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
    ...
    const handleShare = () => {
      ...
      showToast("Public Dossier Link Copied", "success");
    };
                    ...
                    <div className="flex flex-col lg:flex-row items-start lg:items-end -mt-24 lg:mt-0 mb-10 gap-10">
                        ...
                        <div className="flex-grow pb-2 text-left">
                            ...
                            <p className={`font-mono text-sm uppercase tracking-[0.4em] flex items-center gap-3 ${isRuggedMode ? 'text-black font-black' : 'text-slate-500'}`}>
                                <div className={`w-2 h-2 rounded-full ${isRuggedMode ? 'bg-black' : 'bg-red-600 animate-pulse'}`}></div>
                                {user.role.toUpperCase()} SIGNATURE
                            </p>
                        </div>
                        ...
                        <div className="flex gap-4 w-full lg:w-auto">
                             {isEditMode ? (
                                ...
                                <>
                                    <button 
                                        onClick={handleShare}
                                        className={`flex-1 lg:flex-none flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest border px-8 py-4 rounded-2xl transition-all ${isRuggedMode ? 'bg-white border-black text-black border-2' : 'bg-emerald-600 text-white shadow-xl hover:bg-emerald-700'}`}
                                    >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 text-left">
                        <div className="lg:col-span-8 space-y-12">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                                ...
                                <section>
                                    <h3 className={`text-xs font-black uppercase tracking-[0.4em] mb-6 flex items-center gap-3 ${isRuggedMode ? 'text-black font-black' : 'text-slate-500'}`}>
                                        <EnvelopeIcon className={`h-4 w-4 ${isRuggedMode ? 'text-black' : 'text-red-600'}`}/> Email Node
                                    </h3>
                                    {isEditMode ? (
                                        <input type="email" name="email" value={formData.email} onChange={handleInputChange} className={`w-full p-4 border-2 rounded-2xl font-black outline-none italic ${isRuggedMode ? 'bg-white border-black text-black' : 'bg-white/5 border-white/10 text-white focus:border-red-600'}`} placeholder="Enter identity email..." />
                                    ) : (
                                        <p className={`text-xl font-black italic tracking-tighter truncate ${isRuggedMode ? 'text-black' : 'text-white'}`}>{user.email || "NO_UPLINK_SPECIFIED"}</p>
                                    )}
                                </section>
