...
const VitalGauge: React.FC<{ label: string; value: number; color: string; prefix?: string; suffix?: string }> = ({ label, value, color, prefix = '', suffix = '' }) => (
    <div className="bg-slate-900 border border-white/5 p-6 rounded-[2.5rem] shadow-xl group hover:border-blue-500/30 transition-all duration-500 text-left">
        ...
        <p className="text-3xl font-black text-white tracking-tighter italic text-left">
            {prefix}<AnimatedNumber value={value} />{suffix}
        </p>
        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1 text-left">{label}</p>
    </div>
);
...
export const ProjectDetailsPage: React.FC<{ project: Project, onBack: () => void, onNavigate: (tab: ContractorTab) => void }> = ({ project, onBack, onNavigate }) => {
    ...
    return (
        <div className="pb-20 font-sans selection:bg-red-600/30 text-left">
            {/* Ferrari Header */}
            <div className="bg-slate-900 rounded-[3.5rem] shadow-2xl p-12 mb-10 text-white relative overflow-hidden border border-white/10 text-left">
                 ...
                 <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-12 text-left">
                    <div className="flex-grow text-left">
                        ...
                        <h2 className="text-6xl md:text-7xl font-black tracking-tighter uppercase italic text-white leading-none mb-6 text-left">
                            {project.projectName}
                        </h2>
                        <div className="flex flex-wrap gap-8 text-sm font-bold text-slate-400 uppercase tracking-[0.2em] italic opacity-80 text-left">
            ...
            {/* Content Area */}
            <div className="transition-all duration-700 min-h-[600px] animate-fade-in-up text-left">
                {activeTab === 'overview' && (
                  <div className="space-y-10 text-left">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 text-left">
                          ...
                          <div className="space-y-10 lg:col-span-4 text-left">
                              <WeatherWidget />
                ...
                {activeTab === 'chronos' && (
                  <div className="h-[700px] text-left">
                ...
                {activeTab === 'diary' && (
                    <div className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-slate-200 text-left">
