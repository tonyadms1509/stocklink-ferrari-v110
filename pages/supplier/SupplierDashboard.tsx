        ...
        <div id={id} className="bg-slate-900 border border-white/5 p-8 rounded-[2.5rem] shadow-2xl group hover:border-blue-500/30 transition-all duration-500 flex flex-col justify-between relative overflow-hidden text-left">
            ...
            <div className="relative z-10">
                <h3 className="text-5xl font-black text-white mb-2 tracking-tighter italic uppercase">
                    {isCurrency ? (
                      <AnimatedNumber value={Number(numValue)} format={formatCurrency} />
                    ) : (
                      <AnimatedNumber value={isNaN(Number(numValue)) ? 0 : Number(numValue)} />
                    )}
                </h3>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 pl-2 border-l-2 border-blue-600 text-left">{title}</p>
            </div>
        </div>
    );
};
...
export default function SupplierDashboard() {
    ...
    const { orders = [], quoteRequests = [], systemStatus, isGridSaturated } = useData();
    const [isCheckInOpen, setIsCheckInOpen] = useState(false);

    const metrics = useMemo(() => {
        if (!currentCompany) return { revenue: 0, pending: 0, quotes: 0, recent: [] };
        const safeOrders = Array.isArray(orders) ? orders : [];
        const safeQuotes = Array.isArray(quoteRequests) ? quoteRequests : [];
        const myOrders = safeOrders.filter(o => o.supplierId === currentCompany.id);
        ...
        const quotes = safeQuotes.filter(q => q.supplierId === currentCompany.id && q.status === QuoteStatus.Pending).length;
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
                    <div className="text-left">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/10 rounded-xl">
                                ...
                                <p className="text-blue-500 font-mono font-bold text-[10px] tracking-[0.3em] uppercase">Status: {isProduction ? 'MISSION READY' : 'STANDBY'}</p>
                            </div>
                            {isGridSaturated && (
                                <div className="flex items-center gap-2 px-4 py-1.5 bg-red-600/10 border border-red-500/20 rounded-xl animate-pulse">
                                    <ExclamationTriangleIcon className="h-4 w-4 text-red-500" />
                                    <p className="text-red-500 font-mono font-bold text-[10px] tracking-[0.3em] uppercase">GRID SATURATED</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <button onClick={() => setIsCheckInOpen(true)} className="bg-white hover:bg-slate-100 text-slate-950 font-black py-6 px-16 rounded-[2.5rem] flex items-center gap-8 transition-all shadow-2xl transform active:scale-95 group border-4 border-slate-900">
                         ...
                         <div className="text-left border-l-2 border-slate-200 pl-8">
                            ...
                            <span className="block text-[10px] text-slate-500 uppercase font-black tracking-widest">Identify Handshake</span>
                         </div>
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="lg:col-span-2 bg-slate-900 border border-white/10 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-center mb-12 relative z-10 text-left">
                        ...
                        <button className="text-[10px] font-black uppercase text-slate-500 tracking-[0.3em] hover:text-white transition-colors border-b border-white/5 pb-1">All Transmissions</button>
                    </div>
                ...
                <div className="bg-slate-900 border border-white/10 rounded-[3.5rem] p-10 text-white shadow-2xl relative overflow-hidden group flex flex-col justify-between h-full text-left">
                    ...
                    <div className="relative z-10 text-left">
                        ...
                        <p className="text-blue-100 text-lg leading-relaxed mb-10 font-medium italic opacity-70">"National material indices indicate a 12% demand spike in Gauteng Sector Delta. Adjust price anchors to capture regional surplus."</p>
