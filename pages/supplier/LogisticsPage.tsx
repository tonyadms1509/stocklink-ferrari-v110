             ...
             <div className="relative z-10">
                ...
                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Grid Signal: Nominal</span>
                    </div>
                </div>
             </div>
        </div>
...
const PilotCard: React.FC<{ driver: Driver; onRemove: () => void }> = ({ driver, onRemove }) => (
    <div className="bg-slate-900 border border-white/5 p-6 rounded-[2.5rem] flex items-center justify-between group hover:border-blue-500/30 transition-all duration-300 shadow-xl text-left relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
            <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center font-black text-slate-400 text-xl border border-white/5 group-hover:border-red-600/30 transition-all shadow-inner">
        ...
        <div className="flex items-center gap-4 relative z-10">
            <button onClick={onRemove} className="text-slate-600 hover:text-red-500 transition-colors">
             ...
             <div className="p-2 bg-emerald-500/10 rounded-lg shadow-inner border border-emerald-500/20">
...
const LogisticsPage: React.FC = () => {
    ...
    const myVehicles = useMemo(() => {
        ...
        return vehicles.filter(v => v.supplierId === currentCompany.id);
    }, [vehicles, currentCompany]);

    const myDrivers = useMemo(() => {
        if (!currentCompany) return [];
        return drivers.filter(d => d.supplierId === currentCompany.id);
    }, [drivers, currentCompany]);

    const handleInitialSync = async () => {
        setIsSeeding(true);
        try {
            await executeNewclairActivation();
            showToast("UNIT REGISTRY INITIALIZED: FERRARI GRID LIVE", "success");
        } catch (e) {
            showToast("Initialization Protocol Failure", "error");
        } finally {
            setIsSeeding(false);
        }
    };
        ...
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center">
                 <div className="relative w-24 h-24 mb-8">
                    ...
                    <BoltIcon className="absolute inset-0 m-auto h-10 w-10 text-red-600 animate-pulse" />
                </div>
                <p className="text-red-600 font-black uppercase tracking-[0.4em] text-xs animate-pulse">Neural Handshake In Progress</p>
            </div>
        );
    }

    if (myVehicles.length === 0 && myDrivers.length === 0) {
        return (
                    ...
                    <div className="relative z-10">
                        <div className="mb-10 flex flex-col items-center justify-center">
                            <div className="relative w-48 h-48 mb-8">
                                <div className="absolute inset-0 rounded-full border-4 border-blue-500/20 animate-[spin_20s_linear_infinite]"></div>
                                <div className="absolute inset-4 rounded-full border-4 border-blue-400/40 animate-[spin_10s_linear_infinite_reverse]"></div>
                            ...
                            <h2 className="text-6xl font-black text-white italic tracking-tighter uppercase mb-2 leading-none">FLEET <span className="text-blue-500">ISOLATED</span></h2>
                            <p className="text-blue-400 text-xs font-black uppercase tracking-[0.5em] mb-12">Registry Status: Offline</p>
                        </div>
                        
                        <p className="text-slate-400 text-lg mb-12 max-w-2xl mx-auto font-medium italic leading-relaxed">"Local unit registry currently isolated from the national grid. Initialize your node to sync unassigned high-performance chassis and verified pilots for fulfillment orchestration."</p>
                        
                        <div className="flex flex-col sm:flex-row gap-6 justify-center">
                            <button 
                                onClick={handleInitialSync}
                                disabled={isSeeding}
                                className="px-16 py-6 bg-blue-600 text-white font-black rounded-[2rem] text-sm uppercase tracking-[0.4em] transition-all transform active:scale-95 shadow-[0_0_50px_rgba(37,99,235,0.4)] hover:bg-blue-50 disabled:opacity-50 border-4 border-slate-900 flex items-center justify-center gap-4 group"
                            >
                                {isSeeding ? (
                                    <><div className="animate-spin h-6 w-6 border-4 border-white border-t-transparent rounded-full"></div> Synchronizing...</>
                                ) : (
                                    <><BeakerIcon className="h-6 w-6 text-white group-hover:animate-bounce"/> Initialize Registry</>
                                )}
                            </button>
                        </div>
                    </div>
    ...
    return (
        <div className={`h-full flex flex-col space-y-12 font-sans animate-fade-in text-left`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10 border-b border-white/5 pb-10">
                <div className="text-left">
                    <div className="flex items-center gap-3 mb-4 text-blue-500 text-left">
                         <BoltIcon className={`h-6 w-6 ${isNuclearActive ? 'animate-pulse text-blue-400' : 'text-slate-500'}`} />
                         <span className="text-[10px] font-black uppercase tracking-[0.4em]">
                            {isNuclearActive ? 'FERRARI LOGISTICS GRID v80.5' : 'Fleet Management Hub v80.5'}
                         </span>
                    </div>
                    <h2 className="text-7xl font-black text-white tracking-tighter uppercase italic leading-none text-left">UNIT <span className="text-blue-500">REGISTRY</span></h2>
                </div>
                <div className="flex bg-slate-900/50 p-1.5 rounded-[2rem] border border-white/10 shadow-2xl backdrop-blur-xl">
                    ...
                    <button onClick={() => setActiveTab('overview')} className={`px-10 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'overview' ? 'bg-blue-600 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}>Operational Stats</button>
                </div>
            </div>
            ...
            {activeTab === 'fleet' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 animate-fade-in-up">
                     <div className="bg-slate-950 p-10 rounded-[4rem] shadow-2xl relative overflow-hidden group border border-white/5">
                         <div className={`absolute inset-0 bg-carbon opacity-5 animate-pulse`}></div>
                         <div className="flex justify-between items-center mb-10 relative z-10 text-left">
                             <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter flex items-center gap-5">
                                 <TruckIcon className="h-10 w-10 text-blue-500"/> Elite Chassis
                             </h3>
                             <button className="p-4 bg-white/5 hover:bg-white/10 text-white rounded-2xl transition-all border border-white/10 shadow-lg transform active:scale-90">
                         ...
                         <div className="space-y-6 relative z-10">
                             {myVehicles.map(v => (
                                 <UnitCard 
                                    key={v.id} 
                                    vehicle={v} 
                                    onRemove={() => showToast("Unit Locked", "info")}
                     ...
                     <div className="bg-slate-950 p-10 rounded-[4rem] shadow-2xl relative overflow-hidden group border border-white/5">
                         ...
                         <div className="flex justify-between items-center mb-10 relative z-10 text-left">
                             <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter flex items-center gap-5">
                                 <UserGroupIcon className="h-10 w-10 text-blue-500"/> Ace Pilots
                             </h3>
                             <button className="p-4 bg-white/5 hover:bg-white/10 text-white rounded-2xl transition-all border border-white/10 shadow-lg transform active:scale-90">
                                 <PlusIcon className="h-8 w-8"/>
                             </button>
                          ...
                          <div className="space-y-4 relative z-10">
                             {myDrivers.map(d => (
                                 <PilotCard 
                                    key={d.id} 
                                    driver={d} 
                                    onRemove={() => showToast("Pilot De-listed", "info")}
                                 />
                             ))}
                         </div>
                     </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-fade-in-up">
                    <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl border border-white/5 text-left">
                    ...
                    <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl border border-white/5 text-left">
                         ...
                         <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Payload Tempo</p>
                    ...
                    <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl border border-white/5 text-left">
                         ...
                         <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Pilot Integrity</p>
