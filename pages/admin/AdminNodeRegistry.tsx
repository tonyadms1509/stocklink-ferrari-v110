        ...
        <div className="fixed inset-0 bg-slate-950/90 flex items-center justify-center z-[200] p-4 backdrop-blur-md animate-fade-in">
            <div className="bg-slate-900 border border-white/10 rounded-[3rem] shadow-2xl max-w-2xl w-full relative overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-12 relative z-10 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12 text-left">
                        ...
                        <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between text-sm">
                                    <span className="text-slate-500 font-bold uppercase">Referral Code</span>
                                    <span className="text-blue-400 font-mono">{node.referralCode || 'NONE'}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                    ...
                    <div className="space-y-4">
                         <h4 className="text-xs font-black uppercase text-slate-500 tracking-[0.4em] text-left">Audit Actions</h4>
                         <div className="grid grid-cols-2 gap-4">
                            <button 
                                onClick={() => onVerify(node.id, 'verified')}
                                className="bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-2xl text-[10px] uppercase tracking-widest transition-all shadow-xl shadow-emerald-900/40"
                            >
                                Authorize (Verify)
                            </button>
                            <button 
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-black/40 border-b border-white/5">
                            <tr>
                                ...
                                <th className="p-6 font-black text-slate-500 uppercase text-[10px] tracking-widest">Join Hash</th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {filteredNodes.map(node => (
                                <tr key={node.id} className="hover:bg-white/5 transition-colors group">
                                    ...
                                    <td className="p-6">
                                        <span className="text-[10px] font-mono text-slate-600 font-bold">{new Date(node.createdAt || Date.now()).toISOString().split('T')[0]}</span>
                                    </td>
